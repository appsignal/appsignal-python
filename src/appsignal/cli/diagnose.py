...
class DiagnoseCommand(AppsignalCLICommand):
    ...
    # Names where an option's value came from, so that somebody looking at an
    # option that is not what they set can see what set it instead. An option
    # left at its default says nothing, because there is no source to point at.
    def _config_sources_label(self, option: str) -> str:
        ...
        # More than one source holds the option, so each is listed with the
        # value it holds, in the order they are merged. The last one wins.
